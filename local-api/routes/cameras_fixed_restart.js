...
const saveCameras = (cameras) => {
    ...
    // Trigger Go2RTC regen and RESTART
    try {
        const { exec } = require('child_process');
        // RESTART BOTH to be safe ensuring sync
        exec('node /opt/dss-edge/gen_go2rtc_yaml.js && systemctl restart dss-go2rtc', (err, stdout) => {
            if (err) console.error("Go2RTC Regen/Restart Failed:", err);
            else console.log("Go2RTC Config Regenerated & Service Restarted.");
        });
    } catch (e) { }
};
...
router.post("/verify", async (req, res) => {
    ...
    try {
        ...
        if (connected) {
            ...
            try {
                // Call external script with 10s timeout
