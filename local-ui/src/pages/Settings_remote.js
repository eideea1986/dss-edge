...
// Sub-component for Tunnel Settings
function TunnelSettings() {
    const [config, setConfig] = useState({ enabled: false, dispatchHost: "", dispatchApiPort: 0, dispatchVideoPort: 0, status: "Loading..." });

    useEffect(() => {
        API.get("/tunnel").then(res => setConfig(res.data)).catch(() => setConfig({ ...config, status: "Error loading" }));
    }, []);

    const save = async () => {
        try {
            await API.post("/tunnel", config);
            alert("Tunnel settings saved. Service is restarting...");
    ...
    return (
        <div style={{ background: "#252526", padding: 20, border: "1px solid #444", maxWidth: 600 }}>
            <h3 style={{ marginTop: 0, color: "#fff" }}>Tunnel SSH (Reverse Proxy)</h3>
            <div style={{ marginBottom: 15, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span>Status:</span>
                <span style={{ fontWeight: "bold", color: config.status && config.status.includes("Conectat") ? "#4caf50" : "#f44336" }}>
                    {config.status || "Unknown"}
                </span>
            </div>

            <div style={{ marginBottom: 10 }}>
                <label style={{ display: "flex", alignItems: "center", cursor: "pointer", color: "#ddd" }}>
                    <input
                        type="checkbox"
            ...
            <div style={{ opacity: config.enabled ? 1 : 0.5, pointerEvents: config.enabled ? "auto" : "none" }}>
                <div style={{ marginBottom: 10 }}>
                    <label style={{ display: "block", marginBottom: 5, color: "#bbb" }}>Dispatch Server Host/IP</label>
                    <input
                        style={{ ...styles.input, width: "100%" }}
                        value={config.dispatchHost}
                        onChange={e => setConfig({ ...config, dispatchHost: e.target.value })}
                        placeholder="194.107.163.227"
                    />
                </div>
                <div style={{ display: "flex", gap: 15 }}>
                    <div style={{ flex: 1 }}>
                        <label style={{ display: "block", marginBottom: 5, color: "#bbb" }}>API Tunnel Port</label>
                        <input
                            type="number"
                            style={{ ...styles.input, width: "100%" }}
                            value={config.dispatchApiPort}
                            onChange={e => setConfig({ ...config, dispatchApiPort: parseInt(e.target.value) })}
                            placeholder="ex: 21004"
                        />
                    </div>
                    <div style={{ flex: 1 }}>
                        <label style={{ display: "block", marginBottom: 5, color: "#bbb" }}>Video Tunnel Port (Optional)</label>
                        <input
                            type="number"
                            style={{ ...styles.input, width: "100%" }}
                            value={config.dispatchVideoPort}
                            onChange={e => setConfig({ ...config, dispatchVideoPort: parseInt(e.target.value) })}
                            placeholder="ex: 22004"
                        />
                    </div>
                </div>
            </div>
            ...
            <div style={{ marginTop: 20, display: "flex", justifyContent: "flex-end" }}>
                <button style={styles.btnPrimary} onClick={save}>Save & Apply</button>
            </div>
        </div>
    );
}
...
export default function Settings() {
    ...
    const saveEditModal = async () => {
        ...
        try {
            ...
            // PROMPT USER FORCE ADD
            if (window.confirm("Camera connection failed (" + (e.response?.data?.error || e.message) + "). \n\nDo you want to FORCE ADD this camera anyway? It might connect later.")) {
                const cleanCam = { ...currentEditCam };
                // Ensure ID
                if (!cleanCam.id) cleanCam.id = "cam_" + Date.now();

                // Construct fallback URLs if missing
                if (!cleanCam.rtspHd || !cleanCam.rtsp) {
                    const legacyMfgData = DB_MANUFACTURERS.find(m => m.name === cleanCam.manufacturer) || {};
                    const templateMain = legacyMfgData.rtspTemplate || "rtsp://${user}:${pass}@${ip}:554/stream1";
                    cleanCam.rtspHd = templateMain.replace("${user}", cleanCam.user).replace("${pass}", cleanCam.pass).replace("${ip}", cleanCam.ip);
                    cleanCam.rtsp = cleanCam.rtspHd; // Fallback
                }

                const forcedCam = {
                    ...cleanCam,
                    name: cleanCam.name || `Camera ${cleanCam.ip}`,
                    streams: { main: cleanCam.rtspHd, sub: cleanCam.rtsp || cleanCam.rtspHd },
                    capabilities: { codec: "H.264", resolution: "1080p", fps: 25 },
                    recordingMode: currentEditCam.recordingMode || "continuous",
                    motionSensitivity: currentEditCam.motionSensitivity || 50,
                    commands: {},
                    _forceAdded: true
                ...
                const exists = cams.find(c => c.id === currentEditCam.id);
                let newCams;
                if (exists) { newCams = cams.map(c => c.id === currentEditCam.id ? forcedCam : c); }
                else { newCams = [...cams, forcedCam]; }

                try {
                    await API.post("/cameras/config", newCams);
                    loadCams();
                    closeEditModal();
                    alert("Camera force added successfully!");
                } catch (saveErr) {
                    alert("Critical Error Saving Config: " + saveErr.message);
                }
            }
        }
    };
    ...
    const renderContent = () => {
            ...
            return (
                <div style={{ padding: 20 }}>
                    ...
                    <div style={{ background: "#252526", padding: 20, border: "1px solid #444", maxWidth: 700 }}>
                        ...
                        {networkConfig.availableInterfaces && networkConfig.availableInterfaces.length > 0 && (
                            <div style={{ marginBottom: 20 }}>
                                <label style={{ fontSize: 12, color: "#aaa", textTransform: "uppercase", marginBottom: 5, display: "block" }}>Available Interfaces</label>
                                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, background: "#1e1e1e", border: "1px solid #333" }}>
                                    <thead>
                                        <tr style={{ background: "#333", color: "#ddd" }}>
                                            <th style={{ padding: 8, textAlign: "left" }}>Name</th>
                                            <th style={{ padding: 8, textAlign: "left" }}>IP Address</th>
                                            <th style={{ padding: 8, textAlign: "left" }}>MAC</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {networkConfig.availableInterfaces.map(iface => (
                                            <tr key={iface.interface} style={{ borderBottom: "1px solid #333" }}>
                                                <td style={{ padding: 8, color: "#fff", fontWeight: "bold" }}>{iface.interface}</td>
                                                <td style={{ padding: 8, color: "#aaa" }}>{iface.ip} / {iface.netmask}</td>
                                                <td style={{ padding: 8, color: "#888", fontFamily: "monospace" }}>{iface.mac}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
        ...
        if (selection.type === "NETWORK_VPN") {
            ...
            return (
                <div style={{ padding: 20 }}>
                    <h2 style={{ fontSize: 18, marginBottom: 20, color: "#fff" }}>Sub-Network Connections</h2>
