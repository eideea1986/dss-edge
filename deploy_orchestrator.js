...
const localDir = path.join(__dirname, 'orchestrator');
const remoteDir = '/opt/dss-edge/orchestrator';
...
conn.on('ready', () => {
    console.log('SSH Connected. Uploading Orchestrator...');
    conn.sftp(async (err, sftp) => {
        if (err) throw err;
        try {
            await uploadDir(sftp, localDir, remoteDir);
            console.log('✅ ORCHESTRATOR DEPLOY SUCCESSFUL');
        } catch (e) {
            console.error(e);
        } finally {
            conn.end();
